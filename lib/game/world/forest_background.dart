...
class ForestBackground extends PositionComponent {
  ...
  void render(Canvas canvas) {
    ...
    final skyRect = Rect.fromLTWH(0, 0, worldWidth, worldHeight);
    ...
    for (var i = 0; i < 70; i++) {
      ...
      final x = ((i * 7919) % worldWidth.toInt()).toDouble();
    ...
    final canopy = Path()..moveTo(0, groundY);
    for (double x = 0; x <= worldWidth; x += 60) {
      final h = 90 + ((x * 0.13) % 70);
      canopy.lineTo(x, groundY - h);
      canopy.lineTo(x + 30, groundY - h * 0.6);
    }
    canopy.lineTo(worldWidth, groundY);
    canopy.close();
    ...
    for (final x in [90.0, 160.0, worldWidth - 90, worldWidth - 160]) {
      canvas.drawRect(
        Rect.fromLTWH(x - 10, groundY - 180, 20, 180),
        trunkPaint,
      );
      canvas.drawOval(
        Rect.fromCenter(
          center: Offset(x, groundY - 185),
          width: 90,
          height: 120,
        ),
        trunkPaint,
